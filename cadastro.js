const form = document.querySelector('.conteudo_cad')
const nome = document.querySelector('#nome')
const email = document.querySelector('#email')
const tel = document.querySelector('#tel')
const senha = document.querySelector('#senha')
const img_senha = document.querySelector('.ver_ocultar')
const input_cad = document.querySelector('.input_cad')
...
nome.addEventListener('keyup', () => {
    ...
    if(nome.value != '' && resultado == true && tel.value.length == 16 && senha.value.length >= 8 && senha.value.length <= 16) {
        botao.innerHTML = '<input type="submit" name="continuar" value="Continuar" class="botao_cad">'
    } else {
