        ...
        <?php
                ...
                if(isset($_SESSION['cep'])) {
                    ...
                    if(!isset($_SESSION['cep'])) {
                        ...
                        $cep = $_SESSION['cep'];
                        $deAno = $_SESSION['deAno'];
                        $ateAno = $_SESSION['ateAno'];
                        $dePreco = $_SESSION['dePreco'];
                        $atePreco = $_SESSION['atePreco'];
                        $deKm = $_SESSION['deKm'];
                        $ateKm = $_SESSION['ateKm'];
                        ...
                        if($cep != '') {
                            ...
                            $cep = " AND `cidade_usuario`='$endereco->localidade'";
                            
                        }
    
                        if($deAno != '') {
                            $deAno = " AND `anoM_carro`>=$deAno";
                        }
    
                        if($ateAno != '') {
                            $ateAno = " AND `anoM_carro`<=$ateAno";
                        }
    
                        if($dePreco != '') {
                            $dePreco = str_replace('R$ ','',$dePreco);
                            $dePreco = str_replace('.','',$dePreco);
                            $dePreco = " AND `preco_carro`>=$dePreco";
                        }
    
                        if($atePreco != '') {
                            $atePreco = str_replace('R$ ','',$atePreco);
                            $atePreco = str_replace('.','',$atePreco);
                            $atePreco = " AND `preco_carro`<=$atePreco";
                        }
    
                        if($deKm != '') {
                            $deKm = str_replace(' km','',$deKm);
                            $deKm = str_replace('.','',$deKm);
                            $deKm = " AND `km_carro`>=$deKm";
                        }
    
                        if($ateKm != '') {
                            $ateKm = str_replace(' km','',$ateKm);
                            $ateKm = str_replace('.','',$ateKm);
                            $ateKm = " AND `km_carro`<=$ateKm";
                        }
                    }
                        
                    if($pertence != 'nada') {

                        $res_consultar = "SELECT COUNT(`id_fotos`) AS 'valor' FROM `carro` INNER JOIN `fotos` ON `carro`.`id_carro` = `fotos`.`id_carro` INNER JOIN `pertence` ON `carro`.`id_carro` = `pertence`.`id_carro` INNER JOIN `usuario` ON `carro`.`id_usuario` = `usuario`.`id_usuario` INNER JOIN `confirma` ON `carro`.`id_carro` = `confirma`.`id_carro` WHERE `$pertence` = 1 AND `vendido` = 0 AND `maior18` = 1$cep$deAno$ateAno$dePreco$atePreco$deKm$ateKm";
                        $resp_consultar = mysqli_query($conn,$res_consultar);
                        $resp_valor = mysqli_fetch_assoc($resp_consultar);
                        ...
                        if(isset($resp_valor) && $qttd != 0) {
                            ...
                            <div>
                                ...
                                <div class="esportivo">
                                    <img src="img/' . $pertence . '1.png" class="z" width="100%">
                                </div>';
                        ...
                        if ($qttd == 0) {
                            echo '<div class="j">';
                            echo '<form class="cont_erro" action="anuncio.php">';
                            echo '<div class="txt_erro">Sua Pesquisa não teve Resultados</div>';
                            echo '<input type="submit" value="Voltar" class="botao_erro"></input>';
                            echo '</form>';
                            echo '</div>';
                        }
                        ...
                        $res_consultar = "SELECT COUNT(`id_fotos`) AS 'valor' FROM `carro` INNER JOIN `fotos` ON `carro`.`id_carro` = `fotos`.`id_carro` INNER JOIN `pertence` ON `carro`.`id_carro` = `pertence`.`id_carro` INNER JOIN `usuario` ON `carro`.`id_usuario` = `usuario`.`id_usuario` INNER JOIN `confirma` ON `carro`.`id_carro` = `confirma`.`id_carro` WHERE `vendido` = 0 AND `maior18` = 1$cep$deAno$ateAno$dePreco$atePreco$deKm$ateKm";
                        $resp_consultar = mysqli_query($conn,$res_consultar);
                        $resp_valor = mysqli_fetch_assoc($resp_consultar);
                        ...
                        for($i==0; $i<$linhas; $i++) {
                            ...
                            $j = $i*4;
                            for($j==$i*4; $j<($i+1)*4; $j++) {
                                $res_consultar = "SELECT * FROM `carro` INNER JOIN `fotos` ON `carro`.`id_carro` = `fotos`.`id_carro` INNER JOIN `pertence` ON `carro`.`id_carro` = `pertence`.`id_carro` INNER JOIN `usuario` ON `carro`.`id_usuario` = `usuario`.`id_usuario` INNER JOIN `confirma` ON `carro`.`id_carro` = `confirma`.`id_carro` WHERE `vendido` = 0 AND `maior18` = 1$cep$deAno$ateAno$dePreco$atePreco$deKm$ateKm ORDER BY `id_fotos` ASC LIMIT $j,$qttd";
                                $resp_consultar = mysqli_query($conn,$res_consultar);
                                $resp_valor = mysqli_fetch_assoc($resp_consultar);
                    ...
                    echo '<div class="j">';
                    echo '<form class="cont_erro" action="anuncio.php">';
                    echo '<div class="txt_erro">Houve um Erro ao Processar sua Pesquisa</div>';
                    echo '<input type="submit" value="Voltar" class="botao_erro"></input>';
                    echo '</form>';
                    echo '</div>';
                }
