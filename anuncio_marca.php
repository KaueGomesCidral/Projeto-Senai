    ...
    <div class="body_cad">
        ...
        <?php
            ...
            if(isset($_POST['marca'])) {
                $marca = $_POST['marca'];
                $marca = " AND marca_carro LIKE '%$marca%'";
                $_SESSION['marca'] = $marca;
            }

            $res_consultar = "SELECT COUNT(`id_fotos`) AS 'valor' FROM `carro` INNER JOIN `fotos` ON `carro`.`id_carro` = `fotos`.`id_carro` INNER JOIN `pertence` ON `carro`.`id_carro` = `pertence`.`id_carro` INNER JOIN `usuario` ON `carro`.`id_usuario` = `usuario`.`id_usuario` INNER JOIN `confirma` ON `carro`.`id_carro` = `confirma`.`id_carro` WHERE `vendido` = 0 AND `maior18` = 1$marca";
            $resp_consultar = mysqli_query($conn,$res_consultar);
            $resp_valor = mysqli_fetch_assoc($resp_consultar);
            ...
            if($resp_valor && $qttd != 0) {
                ...
                for($i==0; $i<$linhas; $i++) {
                    ...
                    $j = $i*4;
                    for($j==$i*4; $j<($i+1)*4; $j++) {
                        $res_consultar = "SELECT * FROM `carro` INNER JOIN `fotos` ON `carro`.`id_carro` = `fotos`.`id_carro` INNER JOIN `pertence` ON `carro`.`id_carro` = `pertence`.`id_carro` INNER JOIN `usuario` ON `carro`.`id_usuario` = `usuario`.`id_usuario` INNER JOIN `confirma` ON `carro`.`id_carro` = `confirma`.`id_carro` WHERE `vendido` = 0 AND `maior18` = 1$marca ORDER BY `id_fotos` ASC LIMIT $j,$qttd";
                        $resp_consultar = mysqli_query($conn,$res_consultar);
                        $resp_valor = mysqli_fetch_assoc($resp_consultar);
                ...
                echo '<div class="j">';
                echo '<form class="cont_erro" action="index.php">';
